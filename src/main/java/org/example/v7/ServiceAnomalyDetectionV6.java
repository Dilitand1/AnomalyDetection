            ...
            for (int row = 0; row < numRows; row++) {
                ...
                if (row >= values.size()) {
                    throw new IllegalStateException(
                            "Недостаточно данных для подсервиса " +
                                    subServiceNames.get(col) + " в строке " + row
                    );
                }
    ...
    public static void fillMissingValues(Map<String, List<Double>> serviceData) {
        serviceData.forEach((name, values) -> {
            ...
            while (values.size() < maxSize) {
                values.add(mean); // Заполнение средним
            }
        });
    }
        ...
        for (Map.Entry<String, List<Double>> entry : serviceData.entrySet()) {
            if (expectedSize == -1) {
                ...
                throw new IllegalArgumentException(
                        "Несоответствие данных в подсервисе " + entry.getKey() +
                                ". Ожидается: " + expectedSize +
                                ", получено: " + entry.getValue().size()
                );
            }
        }
    }
